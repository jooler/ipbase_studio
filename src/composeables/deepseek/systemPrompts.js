...
【输出格式严格要求】：
1. 你必须将最终配音稿内容和推理过程完全分开
   - 最终配音稿内容只放在content字段中
   - 分析和思考过程只放在reasoning_content字段中
   - 严禁将推理过程或思考步骤混入content字段的最终配音稿中

2. content字段（最终配音稿）要求：
...
3. reasoning_content字段（推理过程）要求：
   - 使用Markdown格式输出你的思考和分析过程
   - 可以使用Markdown标题、列表、粗体等格式元素
   - 包含需求分析、思考过程、创作思路等内容
   - 可使用"## 理解需求"、"## 确定风格"、"## 创作思路"等标题
   - 推理过程仅在reasoning_content中展示，绝不放入content

【特别强调】：
- 推理过程（如"我需要分析用户需求"、"首先理解要点"等）严禁出现在content中
- 确保content只包含最终配音稿纯文本，不包含任何思考步骤或分析过程
- content和reasoning_content必须严格分离，不能交叉或重复
...
【输出格式要求】：
1. 在reasoning_content字段中：
   - 使用Markdown格式分析文案内容和规划分镜思路
   - 说明你的分析过程和创作理念
   - 可以讨论镜头语言、视觉风格和叙事节奏等
