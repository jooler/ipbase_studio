<template>
  <div class="relative-position absolute-full column no-wrap">
    <template v-if="allVideos.length > 0">
      <ArtPlayer :src="currentVideo" :autoplay="false" :options="playerOptions" ref="artPlayerRef"
        @ready="onPlayerReady" @play="onPlay" @pause="onPause" @ended="onVideoEnded" @waiting="onWaiting"
        @src-change="onVideoSrcChange" class="q-space flex flex-center bg-black hidden-controls" />


      <!-- 自定义控制栏 -->
      <div class="custom-controls full-width">
        <!-- 时间轴容器 -->
        <div class="timebar-container" @click="onTimebarClick" @mousemove="onTimebarHover" ref="timebarRef">
          <!-- 总时间轴 -->
          <div class="timebar full-width row items-center gap-xs">
            <!-- 视频分段标记 -->
            <div v-for="(segment, index) in videoSegments" :key="index" class="video-segment"
              :class="{ 'active-segment': currentVideoIndex === index }" :style="{
                left: `${segment.startPercent}%`,
                width: `${segment.widthPercent}%`,
                backgroundColor: index % 2 === 0 ? 'rgba(99, 99, 99, 0.7)' : 'rgba(80, 80, 80, 0.7)',
              }" @click.stop="onSegmentClick($event, index)">
              <!-- 分段进度条 -->
              <div class="segment-progress" :style="{
                width:
    ...
    <div v-else class="q-space flex flex-center">
      <span class="op-5">请点击下方按钮，生成分镜脚本...</span>
    </div>
    <!-- 音频控制界面 -->
    <div class="audio-controls q-px-md q-py-sm bg-dark">
      <div class="row no-wrap items-center gap-sm">
        <!-- 播放/暂停按钮 -->
        <q-btn unelevated round size="md" color="primary" :icon="isPlaying ? 'mdi-pause' : 'mdi-play'"
          @click="togglePlay" />
        ...
        <!-- 音量控制 -->
        <q-btn flat round size="sm" :icon="isMuted ? 'volume_off' : 'volume_up'" color="white" @click="toggleMute" />
        <q-slider v-model="volume" :min="0" :max="100" class="q-mx-sm" style="width: 100px" color="primary"
