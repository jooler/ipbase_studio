<template>
  <div class="column full-height no-wrap">
    <div v-if="studioStore.storyboardCards?.length === 0 && !isGenerating"
      class="flex flex-center full-height text-grey-6">
      <div class="column items-center">
        <q-btn color="deep-orange" size="xl" icon="movie" label="生成分镜" @click="generateStoryboard" />
    ...
    <template v-else>
      <q-scroll-area class="full-width bg-darker q-mt-xs" style="flex: 0 0 124px">
        <div class="column">
          <!-- 分镜卡片区域 -->
          <div class="row items-start no-wrap overflow-auto">
            <StoryboardCard v-for="(card, index) in studioStore.storyboardCards" :key="card.id" :card="card" :class="studioStore.selectedCard?.id === card.id
              ? 'border-solid border-info border-xs'
              : 'border-placeholder'
              " @update:card="updateCard(index, $event)" @search-pexels="searchPexels" @select-card="selectCard" />
          </div>
        </div>
      </q-scroll-area>
      <!-- 卡片详情区域 -->
      <div class="q-space row q-px-xl gap-sm">
        <CardDetail :card="studioStore.selectedCard" @update:card="updateSelectedCard" class="q-space" />
        <!-- 添加控制面板组件 -->
        <TimelineControls :storyboardCards="studioStore.storyboardCards" :isGenerating="isGenerating"
          :isLoadingVideos="isLoadingVideos" :isLoadingImages="isLoadingImages" @regenerate="generateStoryboard"
          @refresh-videos="fetchVideosForCards" @refresh-images="fetchImagesForCards" />
      </div>
    </template>
  </div>
</template>
