<template>
  <div v-if="short" class="column no-wrap gap-sm q-pa-sm">
    <q-responsive v-if="quality[0].url" :ratio="16 / 9">
      <ArtPlayer
        class="fit"
        :src="quality[0].url"
        :quality
    ...
    <q-tab-panels v-model="tab" animated>
      <q-tab-panel name="metadata">
        <q-list dense>
          <q-item>
            <q-item-section side top style="width: 6rem"> 编号： </q-item-section>
            <q-item-section>{{ short.id }}</q-item-section>
          </q-item>
          <q-item>
            <q-item-section side top style="width: 6rem"> 时间： </q-item-section>
            <q-item-section>{{ short.time }}</q-item-section>
          </q-item>
          <q-item>
            <q-item-section side top style="width: 6rem"> 配音： </q-item-section>
            <q-item-section>{{ short.text }}</q-item-section>
          </q-item>
          <q-item>
            <q-item-section side top style="width: 6rem"> 画面描述： </q-item-section>
            <q-item-section>{{ short.description }}</q-item-section>
          </q-item>
          <q-item>
            <q-item-section side top style="width: 6rem"> 转场： </q-item-section>
            <q-item-section>{{ short.transition }}</q-item-section>
          </q-item>
          <q-item>
            <q-item-section side top style="width: 6rem"> 构图： </q-item-section>
            <q-item-section>{{ short.shot_type }}</q-item-section>
          </q-item>
          <q-item>
            <q-item-section side top style="width: 6rem"> 运镜： </q-item-section>
            <q-item-section>{{ short.motion }}</q-item-section>
          </q-item>
          <q-item>
            <q-item-section side top style="width: 6rem"> 关键词： </q-item-section>
      ...
      <q-tab-panel name="file">
        <q-list>
          <q-item v-for="i in quality" :key="i.html">
            <q-item-section>{{
              i.url.split('/').pop().split('.').slice(0, -1).join('.')
            }}</q-item-section>
...
const quality = computed(() => {
  return short.value.selected.video_files
    .map((i, index) => ({
      default: index === 0,
      html: `${i.width}P`,
      url: i.link,
