          ...
          <div class="row items-center full-width" @click="readFile(prop.node.path)">
            ...
            <q-popup-proxy ref="contextMenuRef" context-menu class="shadow-24 radius-sm" @hide="contextMenuHide()">
              <q-list bordered dense class="q-pa-xs radius-sm" style="min-width: 12rem">
                ...
                <q-item clickable v-close-popup class="radius-xs" @click="openInExplorer(prop.node)">
                  <q-item-section side>
                ...
                <q-separator class="op-5 q-my-xs" />
                <q-item v-if="!prop.node.parentId" clickable v-close-popup class="radius-xs text-warning"
                ...
                <q-item v-if="prop.node.parentId" clickable v-close-popup class="radius-xs text-negative"
  ...
  const createFile = async (node, name) => {
    ...
    try {
      ...
      if (node.children) {
        ...
        fileManagerStore.nodeMap[newNode.id] = newNode
      }
    } catch (error) {
  ...
  const deleteItem = async (node) => {
    try {
      ...
      if (node.parentId) {
        ...
        const parentNode = fileManagerStore.nodeMap[node.parentId]
        if (parentNode) {
          parentNode.children = parentNode.children.filter((child) => child.id !== node.id)
        }
      } else {
        // 如果是根节点，从root数组中删除
        fileManagerStore.root = fileManagerStore.root.filter(
      ...
      // 从nodeMap中删除节点
      delete fileManagerStore.nodeMap[node.id]
...
>>>>>>> 8644b82 (修复electron根文件夹不能移除bug)
    ...
    const removeFromNodeMap = (nodeId) => {
      const node = fileManagerStore.nodeMap[nodeId]
      if (node) {
        // 如果有子节点，递归删除
        ...
        // 从nodeMap中删除当前节点
        delete fileManagerStore.nodeMap[nodeId]
      }
    }
  ...
  const refreshFileTree = async () => {
    try {
      ...
      try {
        ...
        fileManagerStore.nodeMap = newNodeMap
