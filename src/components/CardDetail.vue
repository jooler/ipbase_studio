    ...
    <div class="q-space q-pa-xs">
      <q-card flat square class="full-height row transparent">
        ...
        <div class="column q-pa-xs gap-xs">
          <q-btn :flat="activeType !== 'video'" :color="activeType === 'video' ? 'primary' : void 0" icon="mdi-video"
            label="视频素材" :disable="!card.videos?.length" @click="handleMediaTypeChange('video')" />
          <q-btn v-if="false" :flat="activeType !== 'image'" :color="activeType === 'image' ? 'primary' : void 0"
            icon="mdi-image" label="图片素材" :disable="!card.images?.length" @click="handleMediaTypeChange('image')" />
        </div>
        <!-- 视频选择部分 -->
        <q-scroll-area v-if="card.videos && card.videos.length > 0 && activeType === 'video'" horizontal
          class="media-thumbnails q-space">
          <div class="row q-pa-sm">
            <div v-for="(video, idx) in card.videos" :key="idx"
              class="detail-media-thumbnail cursor-pointer col-3 q-pa-xs"
              :class="{ 'media-selected': isVideoSelected(video) }" @click="selectVideo(video)">
        ...
        <!-- 图片选择部分 -->
        <q-scroll-area v-if="card.images && card.images.length > 0 && activeType === 'image'" horizontal
          class="media-thumbnails q-space">
          <div class="row no-wrap q-gutter-xs">
            <div v-for="(image, idx) in card.images" :key="idx" class="detail-media-thumbnail cursor-pointer"
              :class="{ 'media-selected': isImageSelected(image) }" @click="selectImage(image)">
              <img :src="image.src.tiny" :alt="`图片预览 ${idx + 1}`" class="full-width full-height" />
            </div>
          </div>
        </q-scroll-area>
      </q-card>
