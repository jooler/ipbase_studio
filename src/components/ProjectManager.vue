<template>
  <q-scroll-area class="project-manager q-space">
    <q-tree :nodes="treeData" node-key="id" dense color="white" selected-color="white" v-model:selected="selected"
      v-model:expanded="expanded">
      <template v-slot:default-header="prop">
        <div class="row items-center no-wrap text-no-wrap full-width q-pr-xs" :class="currentFile?.id === prop.node.id
          ? 'bg-deep-orange radius-xs border shadow-6'
          : 'border-placeholder'
          ...
          <q-space />
          <div v-if="prop.node.color && prop.node.color.length > 0" class="row items-center no-wrap" style="gap: 2px">
            <q-btn v-for="color in prop.node.color" :key="color" round dense size="0.2rem" :color="color"
              @click.stop="updateNodeColormarker(prop.node.id, color)" />
          </div>
          <q-popup-proxy :ref="(el) => (contextMenuRefs[prop.node.id] = el)" context-menu
            class="no-padding radius-sm z-max">
            <q-list bordered class="q-px-xs q-pb-xs q-pt-none radius-sm">
              <q-item class="no-padding">
                <q-item-section>
                  <q-input v-model="editingText" dense autofocus filled class="radius-xs overflow-hidden border"
                    @update:model-value="updateNode(prop.node.id)" @keyup.enter="finishEditing" />
                </q-item-section>
              </q-item>
              <q-separator class="op-5 q-my-xs" />
              <div class="row items-center full-width justify-between q-py-md q-px-sm">
                <q-btn v-for="i in colors" :key="i" dense round size="0.3rem" :color="i"
                  @click.stop="updateNodeColormarker(prop.node.id, i)" />
              </div>
              <q-separator class="op-5 q-my-xs" />
              <template v-if="prop.node.type === 'folder'">
                <q-item :clickable="!showNewFolderNameInput" :v-ripple="!showNewFolderNameInput"
                  class="q-pa-none radius-xs" @click="createFolderHandler">
                  <q-item-section :class="showNewFolderNameInput ? 'q-pa-none' : 'q-pa-sm'">
                    ...
                    <q-item-label v-else>
                      <q-input v-model="newFolderName" dense autofocus class="radius-xs overflow-hidden border"
                        input-class="q-px-sm" @keyup.enter="createFolder" />
                    </q-item-label>
                  </q-item-section>
                </q-item>
                <q-item :clickable="!showNewFileNameInput" :v-ripple="!showNewFileNameInput" class="q-pa-none radius-xs"
                  ...
                  <q-item-section :class="showNewFileNameInput ? 'q-pa-none' : 'q-pa-sm'">
                    ...
                    <q-item-label v-else>
                      <q-input v-model="newFileName" dense autofocus class="radius-xs overflow-hidden border"
                        input-class="q-px-sm" @keyup.enter="createFile" />
                    </q-item-label>
                  </q-item-section>
                </q-item>
              </template>
    ...
    <q-popup-proxy context-menu ref="rootMenu" class="no-padding radius-sm z-max" @before-show="selected = null">
      <q-list bordered class="q-pa-xs radius-sm">
        <q-item :clickable="!showNewFolderNameInput" :v-ripple="!showNewFolderNameInput" class="q-pa-none radius-xs"
...
<script setup>
  ...
  const { mode, currentFile, jsonContent } = useTts()
  // Storage keys
  ...
  const loadData = async () => {
    try {
      ...
      if (mode.value === 'indexedDB' && (!currentFile.value || !jsonContent.value)) {
  ...
  defineExpose({
    ...
    getCurrentFile,
    updateNodeName,
    loadProjectData, // 暴露loadProjectData方法
  })
</script>
