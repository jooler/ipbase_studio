<template>
  <q-table flat square :rows="rows" :columns="columns" row-key="name" virtual-scroll v-model:pagination="pagination"
    :rows-per-page-options="[0]" class="shadow-0 transparent">
    <template v-slot:body="props">
      <q-tr :props="props" :class="activeItem.id === props.row.id ? 'bg-primary text-white' : ''">
        <q-td key="thumbnial" :props="props" class="cursor-pointer" @click="clickItem(props.row.id)">
          <q-img v-if="props.row.thumbnial" :src="props.row.thumbnial" :ratio="16 / 9" width="80px"
...
<script setup>
  ...
  const columns = [
    { name: 'thumbnial', align: 'center', label: '参考', field: row => row.thumbnial, sortable: false },
    {
      name: 'id',
      required: true,
      label: '编号',
    ...
    { name: 'description', align: 'left', label: '画面描述', field: row => row.description, sortable: false },
    { name: 'transition', align: 'center', label: '转场', field: row => row.transition, sortable: false },
    { name: 'shot_type', align: 'center', label: '构图', field: row => row.shot_type, sortable: false },
    { name: 'motion', align: 'center', label: '运镜', field: row => row.motion, sortable: false },
    { name: 'searchKeywords', align: 'left', label: '关键词', field: row => row.searchKeywords, sortable: false },
  ]
  const rows = computed(() => {
    return storyboardCards.map((i) => ({
      id: i.id,
      thumbnial: i.videos[0]?.image || null,
      time: i.time,
      text: i.text,
