<template>
  <div class="tiptap-ssml-editor">
    ...
    <template v-if="editor">
      <BubbleMenu :editor="editor" :tippy-options="{ duration: 100, maxWidth: 'none' }" class="bubble-menu transparent"
        ...
        <q-card bordered :class="{ 'radius-sm': !hasTextSelection }">
          ...
          <q-card-section class="row no-wrap items-center gap-xs border-top q-pa-xs">
            <template v-if="hasTextSelection">
              <q-btn :color="activeControl === 'rate' ? 'primary' : void 0"
                @click="activeControl = activeControl === 'rate' ? null : 'rate'" icon="speed" unelevated
                :disable="!hasTextSelection">
                <q-tooltip>语速</q-tooltip>
              </q-btn>
              <q-btn :color="activeControl === 'pitch' ? 'primary' : void 0"
                @click="activeControl = activeControl === 'pitch' ? null : 'pitch'" icon="tune" unelevated
                :disable="!hasTextSelection">
                <q-tooltip>音调</q-tooltip>
              </q-btn>
              <q-btn :color="activeControl === 'volume' ? 'primary' : void 0"
                @click="activeControl = activeControl === 'volume' ? null : 'volume'" icon="volume_up" unelevated
                :disable="!hasTextSelection">
                <q-tooltip>音量</q-tooltip>
              </q-btn>
              <q-btn color="green" @click="setSelectedTextAsPreview" icon="record_voice_over" unelevated
                :disable="!hasTextSelection">
                <q-tooltip>设为预览文本</q-tooltip>
              </q-btn>
            </template>
            <q-list v-if="!hasTextSelection" dense class="column gap-xs">
              <q-item clickable class="radius-xs" :class="{ 'bg-primary text-white': currentBreakStrength === 'none' }"
              ...
              <q-item clickable class="radius-xs" :class="{ 'bg-primary text-white': currentBreakStrength === 'weak' }"
            ...
            <q-btn v-if="hasTextSelection" color="negative" flat @click="clearAttributes" icon="backspace"
              :disable="!hasTextSelection">
              <q-tooltip>清除样式</q-tooltip>
            </q-btn>
          </q-card-section>
          <q-card-section v-if="hasTextSelection && activeControl && activeControl !== 'break'"
            class="q-px-md q-pt-lg q-pb-xs border-top">
            <q-slider v-if="activeControl === 'rate'" v-model="selectionRate" :min="50" :max="200" :step="5"
              @update:model-value="applyRate" label :label-value="'语速: ' + selectionRate + '%'" label-always />
            <q-slider v-if="activeControl === 'pitch'" v-model="selectionPitch" :min="50" :max="150" :step="5"
              @update:model-value="applyPitch" label :label-value="'音调: ' + selectionPitch + '%'" label-always />
            <q-slider v-if="activeControl === 'volume'" v-model="selectionVolume" :min="0" :max="100" :step="5"
              @update:model-value="applyVolume" label :label-value="'音量: ' + selectionVolume + '%'" label-always />
          </q-card-section>
        </q-card>
      </BubbleMenu>
    </template>
...
<script setup>
  ...
  watch(
    ...
    () => {
      ...
      const ssml = convertToSsml(jsonContent.value)
      if (!jsonContent.value) {
        ssmlOutput.value = null
        ssmlContent.value = null
        return
      }
      ssmlOutput.value = ssml // 更新本地输出
      ssmlContent.value = ssml // 更新将发送给API的内容

      emit('isEmptyString', editor.value.isEmpty)
    },
    { deep: true },
  )
